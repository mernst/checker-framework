...
When overriding a method, programmers must fully specify the overridding
method, which duplicates information on the overridden method.  An example
is that when definining an \<equals()> method, programmers must write the
type annotation \<@Nullable> and the method postcondition \<@EnsuresNonNullIf>:
