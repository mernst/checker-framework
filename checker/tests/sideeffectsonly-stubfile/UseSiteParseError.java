...
public class UseSiteParseError {
  ...
  // `flowexpr.parse.error.sideeffectsonly` comes from type refinement, which reports the
  // malformed annotation here because `Library` has no declaration under compilation to report it
  // at.
  @SideEffectsOnly("#1")
  void callUnparseable(Library lib) {
    // :: error: (purity.unknown.sideeffectsonly) :: error: (flowexpr.parse.error.sideeffectsonly)
    lib.unparseable();
  }
